...
if (!isDevelopment) {
  ...
  console.log("  widgetJsPath:", widgetJsPath, "exists:", existsSync(widgetJsPath));
  console.log("  widgetCssPath:", widgetCssPath, "exists:", existsSync(widgetCssPath));
  console.log("  widgetTemplatePath:", widgetTemplatePath, "exists:", existsSync(widgetTemplatePath));
}
...
async function buildWidgetHtml() {
  ...
  if (isDevelopment) {
    // In development, bundle on-the-fly using esbuild
    ...
    jsText = (result.outputFiles ?? []).find((file) => file.path.endsWith(".js"))?.text ?? null;
    cssText = (result.outputFiles ?? []).find((file) => file.path.endsWith(".css"))?.text ?? null;
  } else {
    // In production, read prebuilt assets generated by Vite during the build step
    try {
      jsText = readFileSync(widgetJsPath, "utf8");
      console.log(`✓ Loaded widget JS: ${widgetJsPath} (${jsText.length} bytes)`);
    } catch (error) {
  ...
  try {
    ...
    if (prebuiltCss) {
      replacement += `<style>\n${prebuiltCss}\n</style>\n`;
      console.log(`✓ Loaded widget CSS: ${widgetCssPath} (${prebuiltCss.length} bytes)`);
    }
  } catch (error) {
    ...
    if (cssText) {
      replacement += `<style>\n${cssText}\n</style>\n`;
      console.log(`✓ Using esbuild CSS output (${cssText.length} bytes)`);
    } else {
